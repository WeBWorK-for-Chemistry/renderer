...
# There is no need for these to be configurable.
    ...
    unless ( -r $WeBWorK::Constants::WEBWORK_DIRECTORY ) {
        die
"Cannot read webwork root directory at $WeBWorK::Constants::WEBWORK_DIRECTORY";
    }
    unless ( -r $WeBWorK::Constants::PG_DIRECTORY ) {
        die
"Cannot read webwork pg directory at $WeBWorK::Constants::PG_DIRECTORY";
    }
...
sub startup {
    ...
    for (
        qw(problemJWTsecret webworkJWTsecret baseURL formURL SITE_HOST STRICT_JWT)
      )
    {
        $ENV{$_} //= $self->config($_);
    }
...
# convert to absolute URLs
    $ENV{baseURL} = $ENV{SITE_HOST} . $ENV{baseURL}
      unless ( $ENV{baseURL} =~ m|^https?://| );
    $ENV{formURL} = $ENV{baseURL} . $ENV{formURL}
      unless ( $ENV{formURL} =~ m|^https?://| );
   ...
   # Add Cache-Control and Expires headers to static content from webwork2_files
        ...
        $self->hook(
            after_dispatch => sub {
                ...
                if ( $c->req->url->path =~ '/tmp/renderer' ) {
                    ...
                    $c->res->headers->header(
                        Expires => Mojo::Date->new( time + $STATIC_EXPIRES ) );
                }
            }
        );
    }
    ...
    # Helpers
    $self->helper(
        validateRequest => sub { RenderApp::Controller::IO::validate(@_) } );
    $self->helper(
        parseRequest => sub { RenderApp::Controller::Render::parseRequest(@_) }
    );
    $self->helper( croak => sub { RenderApp::Controller::Render::croak(@_) } );
    $self->helper( logID => sub { shift->req->request_id } );
    $self->helper(
        exception => sub { RenderApp::Controller::Render::exception(@_) } );
    ...
    # Static file routes
    $r->any('/webwork2_files/CAPA_Graphics/*static')
      ->to('StaticFiles#CAPA_graphics_file');

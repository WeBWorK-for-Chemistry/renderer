...
async sub writer {
    ...
    my $validatedInput = $c->validateRequest( { required => $required } );
    return unless $validatedInput;
    my $source = Encode::decode( "UTF-8",
        decode_base64( $validatedInput->{problemSource} ) );
...
sub depthSearch_p {
    ...
    my $promise = Mojo::IOLoop->subprocess->run_p(
     ...
     # warn $root_path;
     # warn "TEST";
     # my @subdirs = File::Find::Rule->directory->in( $root_path );
     # warn scalar @subdirs;
     # warn @subdirs;
     # my @symlinkdirs = File::Find::Rule->directory->symlink->in( $root_path );
     # warn scalar @symlinkdirs;
     # warn @symlinkdirs;
     # my @files = File::FInd::Rule->file->in($root_path);
            ...
            my $wanted = sub {
                ...
                warn $depth;
                warn File::Spec::Functions::splitdir($rel);
                $File::Find::prune = 1
            ...
            File::Find::find { wanted => $wanted, no_chdir => 1, follow=>1 }, $root_path;
            return \%all, 200;
        }
    )->catch(
        sub {
...
async sub findNewVersion {
    ...
    for my $seed (@avoidSeeds) {
        my $problem = $c->newProblem(
            { log => $c->log, read_path => $filePath, random_seed => $seed } );
    ...
    for my $i ( 1 .. $maxIterations ) {
        ...
        my $newProblemObj = $c->newProblem(
            { log => $c->log, read_path => $filePath, random_seed => $newSeed }
        );
...
sub _isNewVersion {
    ...
    for my $avoidProblemKey ( keys %$avoidProblems ) {
        ...
        $avoidHTML =~
          s/<input type="hidden" name="problemSeed" value = "\d+">//g;
...
# if DESCRIPTION is only one line, params will not instantiate DESCRIPTION as an array...
    ...
    $incomingTags->{keywords} = [ $incomingTags->{keywords} ]
      unless ( ref( $incomingTags->{keywords} ) =~ /ARRAY/ );

    my $problem =
      $c->newProblem( { log => $c->log, read_path => $incomingTags->{file} } );
    ...
    $problem->load;
    my $header = $1
      if ( $problem->{problem_contents} =~ /(.*?)DOCUMENT\(\);/s );
    $header ||= $problem->{problem_contents}
      ;    # pointer files have no DOCUMENT calls -- others?
    my $return_object = {
        tags              => $tags,
...
sub validate {
    ...
    if ( $v->has_error ) {
        $c->log->error( "Request data failed to validate: "
              . join( ', ', @{ $v->failed } ) );
        my $errMessage = [];
